...
class Search():
    ...
    def calc_search_effectiveness(self):
        ...
        self.sep1 = random.uniform(.2, .9)
        self.sep2 = random.uniform(.2, .9)
        self.sep3 = random.uniform(.2, .9)
    ...
    def revise_target_probs(self):
        '''Update target probabilities based on search effectiveness'''
        denom = (self.p1 * (1 - self.sep1)
                + self.p2 * (1 - self.sep2)
                + self.p3 * (1 - self.sep3))
        self.p1 = self.p1 * (1 - self.sep1) / denom
        self.p2 = self.p2 * (1 - self.sep2) / denom
        self.p3 = self.p3 * (1 - self.sep2) / denom
...
def main():
    ...
    while True:
        ...
        print('\nSearch {} Results 1 = {}'.format(search_num, results_1),
            file=sys.stderr)
        print('Search {} Results 2 = {}\n'.format(search_num, results_2),
            file=sys.stderr)
